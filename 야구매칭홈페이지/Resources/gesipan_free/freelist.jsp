...
<%
	...
	int pageSize = 7;
	int pageCount = fss.getFreePageCount(pageSize);
	int currentPage = Integer.parseInt(strPage); 	
	Vector<GesipanBean> vector = fss1.Free_all_select();	
%>    
...
<link rel="stylesheet" type="text/css" href="../css/free_list.css">
</head>
<body> 	
...
<!-- ========== container ========== -->
		...
		<div id="divContainer">
			...
			<div id="divContent">			
				...
				<div>
					<h2 class="text_align_center">자유게시판</h2>
						<br />
							...
							<tbody>
                                ...
                                %>
                                		<tr onmouseover="this.style.backgroundColor='#DDDDDD';" onmouseout="this.style.backgroundColor='white';">
                                			<td> <%=gb.getIdx() %> </td>
                                			<td>
                                				<a href="../gesipan_free/freeview.jsp?idx=<%=gb.getIdx() %>">
                                					<%= gb.getSubject() %>
                                				</a>
                                			</td>
                                			<td> <%= gb.getDate() %> </td>
							...
							<tr>
								<td align="center">
									<% if(currentPage == 1){ %>
										[이전]
									<%}else { %>
										[<a href="freelist.jsp?page=<%=currentPage - 1%>">이전</a>]
									<%} %>
									...
									<% for(int i = 1 ; i <= pageCount ; i++){  %>
											...
											<%}else{ %>
											    <a style="text-decoration:none" href="freelist.jsp?page=<%=i%>"><%=i %></a>&nbsp;&nbsp;
											<%} %>
									<% }  %>
									...
									<% if(currentPage == pageCount){ %>
										[다음]
									<% }else{ %>
										[<a href="freelist.jsp?page=<%=currentPage + 1%>">다음</a>]
									<% } %>
								</td>						
							</tr>
						</table>
						<!-- 버튼 -->												
							<a href="../gesipan_free/freewrite.jsp"><img id="btnWrite" src="../images/write.gif" ></a>
					</div>
				<!-- 게시판 끝 -->
